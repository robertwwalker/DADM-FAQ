...
Tim.Hortons.GC <- Tim.Hortons.GC %>% mutate(popup = paste0("Tim Horton's <br>",address,"<br>",city,", ",state,"<br>",postal_code,"<br>",toupper(country), sep=""))
head(Tim.Hortons.GC)
```
...
```{r}
Dunkin.Donuts <- Dunkin.Donuts %>% mutate(popup = paste0("Dunkin Donuts <br>",e_address,"<br>",e_state," ", e_postal,"<br>",e_country, sep=""))
...
mp <- leaflet() %>% 
  addTiles() %>%
  addMarkers(~Longitude, ~Latitude, data=US.Map.Coffee, popup=~Address) %>% setView(-71.0382679, 42.3489054, zoom = 8)
mp
```
