...
```{r}
THUS <- Tim.Hortons.GC %>% filter(country=="us") %>% select(country,full.address,lon,lat)
...
```{r}
SBUX <- Starbucks %>% filter(Country=="US") %>% select(Country,`Street Address`,Longitude,Latitude)
names(SBUX) <- c("Country","Address","Longitude","Latitude")
SBUX$chain <- "Starbucks"
head(SBUX)
```

With all three of the data sources in a similar format and the like, it is now time to bind them together.
